 ...
 */
@Data
public class ThreadedHero {

    public Integer no;

    public String name;

    public ThreadedHero left;

    public ThreadedHero right;

    // 0 表示指向的是左子树, 1 表示指向的是前驱节点
    private int leftType;

    // 0 表示指向的是右子树,  1表示指向的是 后继节点
    private int rightType;

    private ThreadedHero parent;

    public ThreadedHero(Integer no, String name) {
        this.no = no;
        this.name = name;
    }
     ...
     * 前序查找
     */
    public ThreadedHero preOrderSearch(int no) {
        System.out.println("前序查找" + this.no);
        if (this.no == no) {
            return this;
        }
        ThreadedHero hero = null;
        if (this.left != null) {
     ...
     * 中序查找
     */
    public ThreadedHero infixOrderSearch(int no) {
     ...
     * 后序查找
     */
    public ThreadedHero postOrderSearch(int no) {
